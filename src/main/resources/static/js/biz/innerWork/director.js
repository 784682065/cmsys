...
var header_out = {
    ...
    saveData: function () {
        header_out.set('applyname').set('duty').set('startoff').set('endoff')
            .set('off_type').set('offdays').set('destination').set('reasons').set('interim_man').set('business');
    ...
    initdata: function (data) {
        ...
        $('#applyname').val(data.initInfo[0].NAME);
        ...
        $('#interim_man').val(data.initInfo[0].INTERIM_MAN);
    ...
    check: function () {
        ...
        if ($.trim($('#interim_man').val()).length == 0) {
            toastr.warning("请填写外出期间临时负责人", "友情提示");
            return true;
        }
